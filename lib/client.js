...
`;
		...
		*/
			...
			while ((m = re.exec(text)) !== null) {
				...
				if (tok.startsWith("**")) out.push((0, react.createElement)("strong", { key }, tok.slice(2, -2)));
					...
					if (mm) {
						...
						out.push(safe ? (0, react.createElement)("a", {
							key,
							href,
							target: "_blank",
							rel: "noreferrer"
		...
		function Panel() {
			...
			const mdCacheRef = (0, react.useRef)(/* @__PURE__ */ new Map());
			const renderMsg = (m) => {
				let cached = mdCacheRef.current.get(m.id);
				if (!cached || cached.text !== m.text) {
					cached = {
						text: m.text,
						node: m.failed === true ? [m.text] : mdToReact(m.text)
					};
		...
		*/
		...
		function OverlayHost({ useSessions }) {
			...
			if (!bus.open) {
				if (bus.sessionId !== "") {
					...
					const s = closedSnap;
					if (!Boolean(s && (s.phase !== "idle" || s.task && s.task !== ""))) return null;
					if (s && s.sessionId !== "" && s.sessionId !== currentSession) return null;
				}
		...
		function apply(ctx) {
			...
			ctx.effect(() => ctx.slots.inject("conversation.session.header.actions", () => ctx.slots.register({
				...
				order: 30
			}, HeaderAction)), "@stars-w/dsh-group-chat: header action");
			ctx.effect(() => ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				name: "shell.overlay",
				id: "dsh-group-chat-panel",
				order: 90
			}, OverlayHost)), "@stars-w/dsh-group-chat: overlay panel");
		}
		//#endregion
		exports.apply = apply;
