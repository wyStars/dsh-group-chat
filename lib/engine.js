...
export class GroupChatEngine {
  ...
  snapshot() {
    return {
      phase: this.phase,
      task: this.task,
      sessionId: typeof this.deps.sessionId === 'string' ? this.deps.sessionId : '', // 群聊归属会话（客户端据此隐藏无关会话的悬浮入口）
    ...
    try {
      while (this.phase === 'discussing') {
        ...
        try {
          ...
          // 循环体保护：未预期异常不得静默退出（否则 finally 会误置 done）
          if (!(err && (err.name === 'AbortError' || /abort/i.test(String(err?.message ?? ''))))) {
            this.phase = 'error'
            this.error = '讨论循环异常：' + String(err?.message ?? err).slice(0, 120)
            this._bump()
          }
          break
        }
...
Default to "direct" unless deep analysis clearly adds value.`
  ...
  /** 构建共享历史字符串（预算截断，时间优先保留最近部分——从后往前消费预算）。 */
  _buildHistory(messages) {
    ...
    for (let i = messages.length - 1; i >= 0; i -= 1) {
      ...
      if (budget <= 0) break
      parts.unshift(line.slice(0, budget))
  ...
  async exportMd() {
    ...
    let path = ''
    let cwd = ''
    let exportError = '' // 仅表达"导出自身"的成败；不影响 this.error（讨论状态错误）
    try {
      ...
      // 注意：导出成功不清 this.error——讨论错误必须保持可见（此前被清空掩盖真实错误）
    } catch (err) {
      exportError = '导出失败：' + String(err?.message ?? err).slice(0, 120)
      this.error = exportError
    }
    this.mdPath = path
    this._bump()
    return { path, content: md, cwd, error: exportError }
  }
