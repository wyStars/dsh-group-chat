...
export class GroupChatEngine {
   ...
   * @param {object|null} [deps.fs] - 宿主 fs 服务（缺省则导出只返回内容）
  ...
  async generateRoles(task, opts = {}) {
    ...
    if (!route) throw new Error('无法解析模型路由：请先在前端选择默认模型')
  ...
  /** 导出 md：拼装任务/角色/纪要/结论 → 写入工作区（fs 可用时）。 */
  async exportMd() {
    ...
    let path = ''
    try {
      const cwd = await this.deps.resolveCwd()
      ...
      if (fs && typeof fs.resolve === 'function' && typeof fs.writeText === 'function') {
        ...
        const target = await fs.resolve(fileName, { cwd })
        await fs.writeText(target, md)
        path = typeof fs.processPath === 'function' ? fs.processPath(target) : fileName
      }
    } catch (err) {
    ...
    this.mdPath = path
    this._bump()
    return { path, content: md, error: this.error || '' }
  }
